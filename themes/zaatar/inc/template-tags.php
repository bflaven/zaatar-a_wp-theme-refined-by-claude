...
function allium_posted_on( $before = '', $after = '' ) {
	...
	// Visible "Updated on" only on singular views and only when the
	// modified day differs from the publish day (freshness signal for
	// readers and crawlers; same-day edits stay a hidden microformat).
	$show_updated = is_singular() && get_the_date() !== get_the_modified_date();
	...
	// Updated On HTML (visible modified date)
	if ( $show_updated ) {
		$html .= sprintf( ' <span class="updated-on">%1$s <time class="updated" datetime="%2$s">%3$s</time></span>',
			esc_html_x( 'Updated on', 'post modified date', 'zaatar' ),
			esc_attr( get_the_modified_date( 'c' ) ),
			esc_html( get_the_modified_date() )
		);
	}
...
if ( ! function_exists( 'allium_reading_time' ) ) :
/**
 * Prints the estimated reading time for the current post
 * (200 words per minute, minimum 1 minute). Posts only.
 */
function allium_reading_time( $before = '', $after = '' ) {

	if ( 'post' !== get_post_type() ) {
		return;
	}

	$text  = wp_strip_all_tags( strip_shortcodes( get_post_field( 'post_content' ) ) );
	...
	$html = sprintf( '<span class="reading-time entry-meta-icon">%s</span>',
		/* translators: %d: estimated reading time in minutes. */
		esc_html( sprintf( __( '%d min read', 'zaatar' ), $minutes ) )
	);

	$html = $before . $html . $after;

	/**
	 * Filters the Reading Time HTML.
	 *
	 * @param string $html Reading Time HTML.
	 */
	$html = apply_filters( 'allium_reading_time_html', $html );
...
/**
 * Display the primary category for a post, falling back to the first category.
 */
function allium_post_first_category( $before = '', $after = '' ) {
    ...
    // 1. Try primary category from BF Primary Category plugin.
     ...
     * @param WP_Term  $cat       The category term used.
     */
	...
	*/
	function allium_post_thumbnail_single( $args = array() ) {
		...
		if ( allium_has_post_thumbnail() ) {
			...
			// Post Thumbnail HTML. Above the fold on singles: this is the
			// LCP element, so load it eagerly at high priority (step 24).
			$html = sprintf( '<figure class="%1$s">%2$s</figure>',
				esc_attr( $args['class'] ),
				get_the_post_thumbnail( null, $args['size'], array(
					'class'         => 'img-featured-single img-responsive',
					'loading'       => 'eager',
					'fetchpriority' => 'high',
				) )
			);
